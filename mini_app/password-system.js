...
class PasswordProtection {
    ...
    bindEvents() {
        // Цифровые кнопки
        document.querySelectorAll('.password-key[data-digit]').forEach(btn => {
    ...
    showError() {
        ...
        if (statusElement) {
            statusElement.innerHTML = '<span class="password-error">Неверный пароль!</span>';
        }
    ...
    clearError() {
        ...
        if (statusElement && currentPasswordElement) {
            statusElement.innerHTML = `Пароль: <span id="currentPassword">${this.currentInput.padEnd(6, '-')}</span>`;
        }
    }
    ...
    updateDisplay() {
        ...
        if (currentPasswordElement) {
            currentPasswordElement.textContent = this.currentInput.padEnd(6, '-');
        }
    }
